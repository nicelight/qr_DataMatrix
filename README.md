...
## Как работает серверная часть и взаимодействие с клиентом и 1С
...
## Важно
- Для интеграции с 1С потребуется отдельная доработка (например, экспорт данных или отдельный API).
- Вся логика учёта и контроля доступа реализована на сервере и в базе MongoDB.
